...
1. Read the nearest `AGENTS.md` (or `CLAUDE.md`, whichever the active client uses) before making repository changes.
2. Create or update the project-root policy file only with concise repository commands, safety boundaries, and verification requirements.
3. Keep secrets, credentials, machine-specific paths, and model choices out of committed policy.

## Choose tools deliberately

Read [the tool reference](references/toolset-guide.md) when identifying components or installation candidates. Do not install every listed tool by default. First establish the use case, available package manager, license, authentication requirements, and compatibility with the active client environment (Codex or Claude Code).
...
3. Configure the server through the active client's documented MCP configuration mechanism (Codex: `codex mcp add`, imperative and unversioned; Claude Code: the `mcpServers` block in `~/.claude/settings.json`, versioned via this dotfiles repo) rather than copying another client's config blindly.
